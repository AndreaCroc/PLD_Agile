...
public class IteratorProbaTransition implements Iterator<Integer> {
	 ...
	 * @param sommetCrt
	 */
	public IteratorProbaTransition(Collection<Integer> nonVus, int sommetCrt, TreeMap<Integer, Integer> mapPredecesseur,Double[][] matricePheromone,Double[][] cout ){
		...
		for (Integer s : nonVus){
                ...
                for (int j = 0; j < num; j++)
                {
                        somme=0.1; //pour quand il reste qu'un élément
                        candidatCrt = candidatsBis[j];
                        coutCrt=cout[sommetCrt][candidatCrt];
			candidats[nbCandidats++] = candidatsBis[j];
                        double res = Math.pow(matricePheromone[sommetCrt][candidatCrt],alpha)*Math.pow((1/coutCrt),beta);
                        for (int i = 0; i < num; i++) {
                            if (i!=j){
                                somme+=Math.pow(matricePheromone[sommetCrt][candidatsBis[i]],alpha)*Math.pow((1/cout[sommetCrt][candidatsBis[i]]),beta);
